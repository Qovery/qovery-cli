...
func renderClusterStatus(clusterStatus *ClusterStatusDto) {
	// Write the header
	fmt.Printf(fmt.Sprintf("%-71s %-20s %-20s %-20s %-20s %-20s\n",
		"",
	...
	for i, node := range sortedNodes {
		...
		// Add a blank line between nodes, but not after the last one
		if !isLastNode {
			fmt.Printf("\n")
		}
	}
...
// ClusterStatusDto represents the status of a cluster
...
// ClusterComputedStatusDto contains computed status information for a cluster
...
// ClusterStatusGlobalStatus represents the global status of a cluster
...
// QoveryComponentInFailure represents a component in failure
type QoveryComponentInFailure struct {
	Type          string `json:"type"`
	ComponentName string `json:"component_name"`
	// Fields for PodInError
	PodName       string                              `json:"pod_name,omitempty"`
	ContainerName string                              `json:"container_name,omitempty"`
	Level         QoveryComponentContainerStatusLevel `json:"level,omitempty"`
	Reason        *string                             `json:"reason,omitempty"`
	Message       *string                             `json:"message,omitempty"`
	// no additional fields needed for MissingComponent as it only has ComponentName
}

// PodInErrorValue represents a pod in error
...
// MissingComponentValue represents a missing component
type MissingComponentValue struct {
	ComponentName string `json:"component_name"`
	Type          string `json:"type"`
}

// QoveryComponentContainerStatusIssue represents an issue with a container
...
// QoveryNodeFailure represents a node failure
type QoveryNodeFailure struct {
	Reason  string `json:"reason"`
	Message string `json:"message"`
}

// QoveryComponentContainerStatusLevel represents the status level of a container
...
// QoveryClusterKubeVersionStatus represents the status of the Kubernetes version
...
// KubeVersionStatusOkValue represents a valid Kubernetes version
type KubeVersionStatusOkValue struct {
	KubeVersion string `json:"kube_version"`
	Type        string `json:"type"`
}

// KubeVersionStatusDriftValue represents a Kubernetes version drift
...
// KubeVersionStatusUnknownValue represents an unknown Kubernetes version status
type KubeVersionStatusUnknownValue struct {
	Type string `json:"type"`
}

// ClusterNodeDto represents a node in a cluster
...
// NodeTaintDto represents a taint on a node
...
// NodeConditionDto represents a condition on a node
...
// NodeResourceDto represents the resources on a node
...
// NodeResourceAllocatedDto represents the allocated resources on a node
type NodeResourceAllocatedDto struct {
	MemoryMib uint32 `json:"memory_mib"`
	CpuMilli  uint32 `json:"cpu_milli"`
}

// NodePodInfoDto represents information about a pod on a node
...
// NodePodErrorStatusDto represents an error status for a container in a pod
...
// PodQoveryServiceInfoDto represents information about a Qovery service in a pod
...
// MetricsUsageDto represents metrics usage information
...
// NodeAddressDto represents an address for a node
type NodeAddressDto struct {
	Type    string `json:"type"`
	Address string `json:"address"`
}

// PvcInfoDto represents information about a persistent volume claim
